...
public class Application {
	...
	public CommandLineRunner demo(ProductRepository productRepository, CategoryRepository categoryRepository, UserRepository userRepository) {
		return (args) -> {
			...
			productRepository.save(new Product("The Fascist Manifesto", user1, 20, 2, categoryRepository.findByName("Books").get(0), ""));
			productRepository.save(new Product("20kg Dumbell", user1, 100, 1, categoryRepository.findByName("Gym & Fitness").get(0), ""));
			productRepository.save(new Product("Mint Cigarettes", user1, 15, 1, categoryRepository.findByName("Banned items").get(0), ""));
